...
/** In-memory state for an OAuth connect session. */
...
interface ProviderTokenRouterOptions {
  httpAuth: HttpAuthService;
  auth: AuthService;
  config: RootConfigService;
  pts: ProviderTokenService;
...
/**
 * Creates an express Router with all provider-token-related endpoints:
 *  - Session interceptors for the connect flow (GET/POST /v1/sessions/:sessionId)
 *  - Provider token API endpoints (/v1/provider-token/*)
 *
 * The session interceptors must be mounted on the main router *before* the
 * OidcRouter so they match first; sessions not found in `connectStates` call
 * `next()` to fall through to the OidcRouter.
 *
 * Returns two routers:
 *  - `sessionRouter` – session interceptors (mount before OidcRouter)
 *  - `apiRouter` – /v1/provider-token/* endpoints (mount after OidcRouter)
 */
export function createProviderTokenRouters(options: ProviderTokenRouterOptions) {
  const { httpAuth, auth, config, pts, logger, appUrl, authUrl } = options;

  // In-memory state for OAuth connect flow (PoC – production would use DB/cache)
  const connectStates = new Map<string, ConnectState>();
  ...
  // ---------------------------------------------------------------
  // Session interceptors (mount before OidcRouter on the main router)
  // ---------------------------------------------------------------
  const sessionRouter = Router();

  sessionRouter.get('/v1/sessions/:sessionId', async (req, res, next) => {
  ...
  apiRouter.get('/v1/provider-token', async (req, res) => {
    const credentials = await httpAuth.credentials(req, {
      allow: ['service', 'user'],
    });
  ...
  apiRouter.post('/v1/provider-token/grant', async (req, res) => {
    const credentials = await httpAuth.credentials(req, {
      allow: ['user'],
    });
  ...
  // Revoke consent
  apiRouter.delete('/v1/provider-token/grant', async (req, res) => {
  ...
  apiRouter.get('/v1/provider-token/grants', async (req, res) => {
    const credentials = await httpAuth.credentials(req, {
      allow: ['user'],
    });
    const userEntityRef = credentials.principal.userEntityRef;
  ...
  apiRouter.get('/v1/provider-token/connect', async (req, res) => {
    const providerId = req.query.provider as string;
    const pluginId = req.query.plugin as string;
  ...
  return { sessionRouter, apiRouter };
}
