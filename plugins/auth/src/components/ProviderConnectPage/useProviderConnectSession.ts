...
interface ProviderConnectSession {
  id: string;
  clientName?: string;
  clientId: string;
  scope?: string;
...
type ProviderConnectState =
  ...
  | { status: 'loaded'; session: ProviderConnectSession }
  | {
      status: 'submitting';
...
export const useProviderConnectSession = (opts: { sessionId?: string }) => {
  ...
  const sessionState = useAsync(async () => {
    ...
    return (await response.json()) as ProviderConnectSession;
  }, [sessionId]);

  const [actionState, handleActionInternal] = useAsyncFn(
    async (action: 'approve' | 'reject', session: ProviderConnectSession) => {
      const baseUrl = await discoveryApi.getBaseUrl('auth');
      const response = await fetchApi.fetch(
  ...
  const getState = (): ProviderConnectState => {
    ...
    if (sessionState.error) {
      return {
        ...
        error: isError(sessionState.error)
          ...
          : 'Failed to load provider connect request',
      };
    }
